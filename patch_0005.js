...
/***/function (module, exports, __webpack_require__) {
	...
	function setupSocket(socket) {
		...
		socket.on('gameSetup', function (data, mockupData, setup) {
			// Initalization call
       ...
       window.cancelAnimationFrame(global.animLoopHandle);
		...
		socket.on('syncback', function (clientTime, serverTime) {
			...
			if (sync.length < 20) {
				...
				global.messages = "Syncing clocks, " + sync.length + "/20...";
			} else {
	...
	function drawEntity(x, y, instance, ratio) {
		...
		var turretInfo = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
		var blend = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : instance;
		...
		if (source.turrets.length === m.turrets.length) {
			for (var i = 0; i < m.turrets.length; i++) {
				var t = m.turrets[i];
				if (t.layer === 0) {
					var ang = t.direction + rot,
					    len = t.offset * drawSize / instance.size;
					drawEntity(xx + len * Math.cos(ang), yy + len * Math.sin(ang), t, ratio, scale, rot + source.turrets[i].facing, context, source.turrets[i], blend);
				}
			}
		} else {
	...
	function gameDraw(ratio) {
		...
		renderTimes++;
		try {
			interpolateMotion = function interpolateMotion(v1, v2, t) {
				var tt = 1 + t / metrics.rendergap;
			...
			var t = gui.fps * Math.min(100, Math.max(-100, global.time - player.time - Math.min(metrics.rendergap, 75)));

			{
				// Move the camera
				player.renderx = t >= 0 ? player.x + extrapolateMotion(player.lastvx, player.vx, t) : player.lastx + (player.x - player.lastx) * interpolateMotion(player.lastvx, player.vx, t);
				player.rendery = t >= 0 ? player.y + extrapolateMotion(player.lastvy, player.vy, t) : player.lasty + (player.y - player.lasty) * interpolateMotion(player.lastvy, player.vy, t);
...

			{
				...
				entities.forEach(function (instance) {
					instance.render.x = t >= 0 ? instance.x + extrapolateMotion(instance.render.lastvx, instance.vx, t) : instance.render.lastx + (instance.x - instance.render.lastx) * interpolateMotion(instance.render.lastvx, instance.vx, t);
					instance.render.y = t >= 0 ? instance.y + extrapolateMotion(instance.render.lastvy, instance.vy, t) : instance.render.lasty + (instance.y - instance.render.lasty) * interpolateMotion(instance.render.lastvy, instance.vy, t);
					instance.render.f = instance.twiggle ? Math.atan2(target.y, target.x) : instance.render.f = instance.render.lastf + (1 + t / metrics.rendergap) * angleDifference(instance.render.lastf, instance.facing);
...

			{
				...
				var len = 0;
				var height = 18;
				var _x14 = global.screenWidth / 2;
				var _y = spacing;
				for (var _i4 = messages.length - 1; _i4 >= 0; _i4--) {
					var msg = messages[_i4],
					...
					drawBar(_x14 - len / 2, _x14 + len / 2, _y + height / 2, height, color.black);

					ctx.globalAlpha = Math.min(1, msg.alpha);
					ctx.textAlign = 'center';
					drawText(text, _x14, _y + height - 4, height - 4, color.guiwhite);
					...
					if (msg.status > 1) {
						messages[_i4].status -= 0.05;
						messages[_i4].alpha += 0.05;
					} else if (_i4 === 0 && (messages.length > 5 || getNow() - msg.time > 10000)) {
						messages[_i4].status -= 0.05;
						messages[_i4].alpha -= 0.05;
						if (messages[_i4].alpha <= 0) {
							messages.splice(0, 1);
						}
					}
				}
...

			{
				...
				gui.skills.typeData.forEach(function (skill) {
					...
					if (cap) {
						...
						if (extension) {
							_len = _len * (cap + 1) / _max;
							_max = cap + 1;
						}
						drawBar(_x15 + _height / 2, _x15 - _height / 2 + _len, _y2 + _height / 2, _height, color.black);
						drawBar(_x15 + _height / 2, _x15 + _height / 2 + (_len - _height) * (cap / _max), _y2 + _height / 2, _height - 3, color.grey);
						drawBar(_x15 + _height / 2, _x15 + _height / 2 + (_len - _height) * (level / _max), _y2 + _height / 2, _height - 3.5, getColor(col));
						if (blocking) {
							// Blocked-off area
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x15 + (_len - _height) * j / _max, _y2 + 1.5, _x15 + (_len - _height) * j / _max, _y2 - 3 + _height);
							}
						}
						ctx.strokeStyle = color.black; // Vertical dividers
						ctx.lineWidth = 1;
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x15 + (_len - _height) * _j / _max, _y2 + 1.5, _x15 + (_len - _height) * _j / _max, _y2 - 3 + _height);
						}
						...
						drawText(name, Math.round(_x15 + _len / 2) + 0.5, Math.round(_y2 + _height - 5) + 0.5, _height - 5, textcolor);
						ctx.textAlign = 'right'; // Skill key
						drawText('[' + ticker % 10 + ']', Math.round(_x15 + _len - _height * 0.25) - 0.5, Math.round(_y2 + _height - 6) + 0.5, _height - 5, textcolor);
						ctx.textAlign = 'left'; // Skill value
						drawText(value, Math.round(_x15 + _len + 4) + 0.5, Math.round(_y2 + _height - 5) + 0.5, _height - 3, getColor(col));
				...
				if (gui.skills.points !== 0) {
					...
					drawText('x' + gui.skills.points, Math.round(_x15 + _len - 2) + 0.5, Math.round(_y2 + _height - 4) + 0.5, 20, color.guiwhite);
				}
			}

			{
				...
				var _height2 = 25;
				var _x16 = (global.screenWidth - _len2) / 2;
				var _y3 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				drawBar(_x16, _x16 + _len2, _y3 + _height2 / 2, _height2, color.black);
				drawBar(_x16, _x16 + _len2, _y3 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x16, _x16 + _len2 * gui.progress, _y3 + _height2 / 2, _height2 - 3.5, color.gold);
				ctx.lineWidth = 2;
				ctx.textAlign = 'center';
				drawText('Level ' + gui.level + ' ' + gui.type, Math.round(_x16 + _len2 / 2) + 0.5, Math.round(_y3 + _height2 - 6) + 0.5, _height2 - 4, color.guiwhite);
				_height2 = 14;
				_y3 -= _height2 + _vspacing2;
				drawBar(_x16, _x16 + _len2, _y3 + _height2 / 2, _height2, color.black);
				drawBar(_x16, _x16 + _len2, _y3 + _height2 / 2, _height2 - 3, color.grey);
				var scoreFactor = max ? Math.min(1, gui.score / max) : 1;
				drawBar(_x16, _x16 + _len2 * scoreFactor, _y3 + _height2 / 2, _height2 - 3.5, color.green);
				ctx.lineWidth = 2;
				ctx.textAlign = 'center';
				drawText('Score: ' + handleLargeNumber(gui.score), Math.round(_x16 + _len2 / 2) + 0.5, Math.round(_y3 + _height2 - 4) + 0.5, _height2 - 2, color.guiwhite);
				ctx.lineWidth = 4;
				drawText(strDecodeUTF16(player.name), Math.round(_x16 + _len2 / 2) + 0.5, Math.round(_y3 - 10 - _vspacing2) + 0.5, 32, color.guiwhite);
			}

			{
				...
				var _height3 = _len3;
				var _x17 = global.screenWidth - _len3 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x17 + _i5 * _len3 / _W, _y4 + j++ * _height3 / _H, _len3 / _W, _height3 / _H);
					});
					_i5++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x17, _y4, _len3, _height3);
				ctx.globalAlpha = 0.8;
				minimap.forEach(function (o) {
					ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
					drawGuiRect(_x17 + o[0] / global.gameWidth * _len3, _y4 + o[1] / global.gameHeight * _height3, 1, 1);
				});
				ctx.globalAlpha = 1;
				ctx.lineWidth = 1;
				ctx.strokeStyle = color.black;
				drawGuiRect( // My position
				_x17 + player.x / global.gameWidth * _len3 - 1, _y4 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				drawGuiRect(_x17, _y4, _len3, _height3, true); // Border

				ctx.textAlign = 'right';
				drawText('Update Rate: ' + metrics.updatetime + 'Hz', _x17 + _len3, _y4 - 52, 10, color.guiwhite);
				drawText('Latency: ' + metrics.latency + 'ms', _x17 + _len3, _y4 - 38, 10, color.guiwhite);
				drawText('Client FPS: ' + metrics.rendertime, _x17 + _len3, _y4 - 24, 10, color.guiwhite);
				drawText('Server Speed: ' + (100 * gui.fps).toFixed(2) + '%', _x17 + _len3, _y4 - 10, 10, color.guiwhite);
			}

			{
				...
				ctx.lineWidth = 4;
				ctx.textAlign = 'center';
				drawText('Leaderboard:', Math.round(_x18 + _len4 / 2) + 0.5, Math.round(_y5 - 6) + 0.5, _height4 + 4, color.guiwhite);
				var _i6 = 0;
				leaderboard.forEach(function (entry) {
					leaderboardScore[_i6] += (entry.score - leaderboardScore[_i6]) / 20;
					drawBar(_x18, _x18 + _len4, _y5 + _height4 / 2, _height4, color.black);
					drawBar(_x18, _x18 + _len4, _y5 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x18, _x18 + _len4 * shift, _y5 + _height4 / 2, _height4 - 3.5, getColor(entry.bar));
					...
					drawText(txt + lab + ': ' + handleLargeNumber(Math.round(leaderboardScore[_i6++])), Math.round(_x18 + _len4 / 2) + 0.5, Math.round(_y5 + _height4 - 5) + 0.5, _height4 - 5, color.guiwhite);
					...
					var picture = getEntityImageFromMockup(entry.index, entry.color),
					    ...
					    xx = _x18 - 1.5 * _height4 - scale * position.middle.x * 0.707,
...

			{
				...
				if (gui.upgrades.length > 0) {
					...
					gui.upgrades.forEach(function (model) {
						...
						drawGuiRect(_x19, _y6, _len5, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x19, _y6, _len5, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x19, _y6 + _height5 * 0.6, _len5, _height5 * 0.4);
						ctx.globalAlpha = 1;
						...
						var picture = getEntityImageFromMockup(model[0], model[1]),
						    ...
						    xx = _x19 + 0.5 * _len5 - scale * position.middle.x * Math.cos(upgradeSpin),
						...
						ctx.lineWidth = 2;
						drawText(picture.name, Math.round(_x19 + 0.9 * _len5 / 2) + 0.5, Math.round(_y6 + _height5 - 6) + 0.5, _height5 / 8 - 3, color.guiwhite);
						ctx.textAlign = 'right'; // Upgrade key
						drawText('[' + getClassUpgradeKey(_ticker) + ']', Math.round(_x19 + _len5 - 4) + 0.5, Math.round(_y6 + _height5 - 6) + 0.5, _height5 / 8 - 3, color.guiwhite);

						ctx.strokeStyle = color.black; // Already incremented, see above
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x19, _y6, _len5, _height5, true); // Border

						if (_ticker++ % 2) {
							_y6 -= _height5 + internalSpacing;
							_x19 += _len5 + internalSpacing;
						} else {
							_y6 += _height5 + internalSpacing;
						}
					});

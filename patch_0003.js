...
/***/function (module, exports, __webpack_require__) {
	...
	function startGame(type) {
		window.localStorage.playerName = global.playerName = playerNameInput.value.replace(/(<([^>]+)>)/ig, '').substring(0, 25);
		window.localStorage.playerKey = global.playerKey = playerKeyInput.value.replace(/(<([^>]+)>)/ig, '').substring(0, 64);
	...
	window.onload = function () {
		playerNameInput.value = window.localStorage.playerName || "";
		playerKeyInput.value = window.localStorage.playerKey || "";
	...
	function gameDraw(ratio) {
		...
		renderTimes++;
		try {
			var interpolateMotion = function interpolateMotion(v1, v2, t) {
				var tt = 1 + t / metrics.rendergap;
				return v1 === -v2 ? tt : tt * (v1 + 0.5 * tt * (v2 - v1)) / (v1 + 0.5 * (v2 - v1));
			};

			var extrapolateMotion = function extrapolateMotion(v1, v2, t) {
				var tt = 30 * t / 1000,
				    drop = Math.max(0, Math.min(1, 1.5 - 1.5 * t / 1000));
				return (v2 + t / metrics.rendergap * (v2 - v1)) * tt * drop;
			};
...

			{
				...
				entities.forEach(function (instance) {
					...
					instance.render.f = instance.twiggle ? Math.atan2(target.y, target.x) : instance.render.f = instance.render.lastf + (1 + t / metrics.rendergap) * (instance.facing - instance.render.lastf);
	...
	function animloop() {
		...
		var ratio = config.graphical.screenshotMode ? 0.5 : Math.max(global.screenWidth / player.renderv, global.screenHeight / player.renderv / 9 * 16);

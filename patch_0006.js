...
/***/function (module, exports, __webpack_require__) {
	...
	function convertRawData(data) {
		function process(i) {
			var isTurret = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
		...
		return data.map(function (datum) {
			datum = process(datum);
			var e = entities.find(function (x) {
			...
			if (e != null) {
				...
				e.render = {
					x: datum.x,
					y: datum.y,
					lastx: datum.x - datum.vx,
					lasty: datum.y - datum.vy,
					lastvx: datum.vx,
					lastvy: datum.vy,
					lastf: datum.facing,
					f: datum.facing,
	...
	function setupSocket(socket) {
		...
		socket.on('gameSetup', function (data, mockupData, setup, speed) {
			// Initalization call
			...
			mockups = mockupData;
			roomSetup = setup;
			config.roomSpeed = speed;
			socket.emit('sync', getNow());
		});
		socket.on('uplink', function (camera, entityList, newGui) {
			...
			if (camera.time > player.time || player.time == null) {
				...
				metrics.rendergap = camera.time - player.time;
       ...
       window.cancelAnimationFrame(global.animLoopHandle);
		...
		socket.on('syncback', function (clientTime, serverTime) {
			var laten = getNow() - clientTime,
			    delta = Math.round(getNow() - serverTime + laten / 2);
			...
			if (sync.length < 10) {
				...
				global.messages = "Syncing clocks, please do not tab away. " + sync.length + "/10...";
			} else {
				sync.sort(function (e, f) {
					return e.latency - f.latency;
				});
				var median = sync[9].latency,
				    sd = 0,
				    sum = 0,
				    valid = 0;
	...
	function drawEntity(x, y, instance, ratio) {
		...
		var blend = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : instance;

		context = assignedContext ? assignedContext : ctx;
		var fade = blend.fade,
		    drawSize = scale * ratio * instance.size * (1 + 0.5 * (1 - fade)),
		...
		if (source.turrets.length === m.turrets.length) {
			for (var i = 0; i < m.turrets.length; i++) {
				...
				if (t.layer === 0) {
					...
					drawEntity(xx + len * Math.cos(ang), yy + len * Math.sin(ang), t, ratio, drawSize / ratio / t.size * t.sizeFactor, source.turrets[i].facing + turretsObeyRot * rot, turretsObeyRot, context, source.turrets[i], blend);
				}
			}
		} else {
			throw "Mismatch turret number with mockup.";
		}
		// Draw guns
		context.lineWidth = Math.max(2, ratio * 4);
		context.fillStyle = mixColors(color.grey, blend.blend.color, blend.blend.amount);
		context.strokeStyle = color.black;
		if (source.guns.length === m.guns.length) {
			for (var _i3 = 0; _i3 < m.guns.length; _i3++) {
				var g = m.guns[_i3],
				    position = source.guns[_i3],
		...
		// Draw body
		var sides = instance.shape ? instance.shape : 25 + Math.round(drawSize / 50); // Default sides for a circle shall be 30.
		context.globalAlpha = 1;
		context.fillStyle = mixColors(getColor(instance.color), blend.blend.color, blend.blend.amount);
		...
		if (source.turrets.length === m.turrets.length) {
			for (var _i4 = 0; _i4 < m.turrets.length; _i4++) {
				var _t = m.turrets[_i4];
				if (_t.layer === 1) {
					var _ang = _t.direction + _t.angle + rot,
					    _len = _t.offset * drawSize;
					drawEntity(xx + _len * Math.cos(_ang), yy + _len * Math.sin(_ang), _t, ratio, drawSize / ratio / _t.size * _t.sizeFactor, source.turrets[_i4].facing + turretsObeyRot * rot, turretsObeyRot, context, source.turrets[_i4], blend);
				}
			}
		} else {
			throw "Mismatch turret number with mockup.";
	...
	function gameDraw(ratio) {
		...
		renderTimes++;
		try {
			var timediff = global.time - player.time,
			    t = Math.max(timediff - 100, -metrics.rendergap);
			if (t > 500) {
				t = 500 * 500 * Math.sin(t / 500 - 1) / t + 500;
			}
			var tt = t / metrics.rendergap,
			    ts = config.roomSpeed * 30 * t / 1000;

			interpolateMotion = function interpolateMotion(p1, p2, v1, v2) {
				var k = Math.cos((1 + tt) * Math.PI);
				return 0.5 * (((1 + tt) * v1 + p1) * (k + 1) + (-tt * v2 + p2) * (1 - k));
			};

			extrapolateMotion = function extrapolateMotion(v1, v2) {
				return (v2 + 0.5 * t / metrics.rendergap * (v2 - v1)) * ts;
			};

			{
				// Move the camera
				var desiredx = t >= 0 ? player.x + extrapolateMotion(player.lastvx, player.vx) : interpolateMotion(player.lastx, player.x, player.lastvx, player.vx),
				    desiredy = t >= 0 ? player.y + extrapolateMotion(player.lastvy, player.vy, t) : interpolateMotion(player.lasty, player.y, player.lastvy, player.vy),
				    smooth = 1; //(metrics.rendergap > 100) ? 0.25 : 1;
				player.renderx += smooth * (desiredx - player.renderx);
				player.rendery += smooth * (desiredy - player.rendery);
...

			{
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						var left = Math.max(0, ratio * i * global.gameWidth / W - px + global.screenWidth / 2),
...

			{
				...
				entities.forEach(function (instance) {
					instance.render.x = t >= 0 ? instance.x + extrapolateMotion(instance.render.lastvx, instance.vx, t) : interpolateMotion(instance.render.lastx, instance.x, instance.render.lastvx, instance.vx, t);
					instance.render.y = t >= 0 ? instance.y + extrapolateMotion(instance.render.lastvy, instance.vy, t) : interpolateMotion(instance.render.lasty, instance.y, instance.render.lastvy, instance.vy, t);
					instance.render.f = instance.twiggle === -1 ? Math.atan2(target.y, target.x) : instance.twiggle === -1 ? instance.render.f = instance.render.lastf + t * 0.02 * 30 / 1000 : instance.render.f = instance.render.lastf + (1 + t / metrics.rendergap) * angleDifference(instance.render.lastf, instance.facing);
					var x = instance.twiggle === -1 ? 0 : ratio * instance.render.x - px,
					    y = instance.twiggle === -1 ? 0 : ratio * instance.render.y - py;
					x += global.screenWidth / 2;
					y += global.screenHeight / 2;
...

			{
				...
				var len = 0;
				var height = 18;
				var _x15 = global.screenWidth / 2;
				var _y = spacing;
				for (var _i5 = messages.length - 1; _i5 >= 0; _i5--) {
					var msg = messages[_i5],
					...
					drawBar(_x15 - len / 2, _x15 + len / 2, _y + height / 2, height, color.black);

					ctx.globalAlpha = Math.min(1, msg.alpha);
					ctx.textAlign = 'center';
					drawText(text, _x15, _y + height - 4, height - 4, color.guiwhite);
					...
					if (msg.status > 1) {
						messages[_i5].status -= 0.05;
						messages[_i5].alpha += 0.05;
					} else if (_i5 === 0 && (messages.length > 5 || getNow() - msg.time > 10000)) {
						messages[_i5].status -= 0.05;
						messages[_i5].alpha -= 0.05;
						if (messages[_i5].alpha <= 0) {
							messages.splice(0, 1);
						}
					}
				}
...

			{
				...
				var _vspacing = 4;
				var _len2 = alcoveSize * global.screenWidth; // The 30 is for the value modifiers
				var save = _len2;
				var _height = 15;
				var _x16 = spacing;
				...
				gui.skills.typeData.forEach(function (skill) {
					...
					if (cap) {
						...
						if (extension) {
							_len2 = _len2 * (cap + 1) / _max;
							_max = cap + 1;
						}
						drawBar(_x16 + _height / 2, _x16 - _height / 2 + _len2, _y2 + _height / 2, _height, color.black);
						drawBar(_x16 + _height / 2, _x16 + _height / 2 + (_len2 - _height) * (cap / _max), _y2 + _height / 2, _height - 3, color.grey);
						drawBar(_x16 + _height / 2, _x16 + _height / 2 + (_len2 - _height) * (level / _max), _y2 + _height / 2, _height - 3.5, getColor(col));
						if (blocking) {
							// Blocked-off area
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x16 + (_len2 - _height) * j / _max, _y2 + 1.5, _x16 + (_len2 - _height) * j / _max, _y2 - 3 + _height);
							}
						}
						ctx.strokeStyle = color.black; // Vertical dividers
						ctx.lineWidth = 1;
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x16 + (_len2 - _height) * _j / _max, _y2 + 1.5, _x16 + (_len2 - _height) * _j / _max, _y2 - 3 + _height);
						}
						...
						drawText(name, Math.round(_x16 + _len2 / 2) + 0.5, Math.round(_y2 + _height - 5) + 0.5, _height - 5, textcolor);
						ctx.textAlign = 'right'; // Skill key
						drawText('[' + ticker % 10 + ']', Math.round(_x16 + _len2 - _height * 0.25) - 0.5, Math.round(_y2 + _height - 6) + 0.5, _height - 5, textcolor);
						ctx.textAlign = 'left'; // Skill value
						drawText(value, Math.round(_x16 + _len2 + 4) + 0.5, Math.round(_y2 + _height - 5) + 0.5, _height - 3, getColor(col));
				...
				if (gui.skills.points !== 0) {
					...
					drawText('x' + gui.skills.points, Math.round(_x16 + _len2 - 2) + 0.5, Math.round(_y2 + _height - 4) + 0.5, 20, color.guiwhite);
				}
			}

			{
				...
				var _vspacing2 = 4;
				var _len3 = 2 * alcoveSize * global.screenWidth;
				var _height2 = 25;
				var _x17 = (global.screenWidth - _len3) / 2;
				var _y3 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				drawBar(_x17, _x17 + _len3, _y3 + _height2 / 2, _height2, color.black);
				drawBar(_x17, _x17 + _len3, _y3 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x17, _x17 + _len3 * gui.progress, _y3 + _height2 / 2, _height2 - 3.5, color.gold);
				ctx.lineWidth = 2;
				ctx.textAlign = 'center';
				drawText('Level ' + gui.level + ' ' + gui.type, Math.round(_x17 + _len3 / 2) + 0.5, Math.round(_y3 + _height2 - 6) + 0.5, _height2 - 4, color.guiwhite);
				_height2 = 14;
				_y3 -= _height2 + _vspacing2;
				drawBar(_x17, _x17 + _len3, _y3 + _height2 / 2, _height2, color.black);
				drawBar(_x17, _x17 + _len3, _y3 + _height2 / 2, _height2 - 3, color.grey);
				var scoreFactor = max ? Math.min(1, gui.score / max) : 1;
				drawBar(_x17, _x17 + _len3 * scoreFactor, _y3 + _height2 / 2, _height2 - 3.5, color.green);
				ctx.lineWidth = 2;
				ctx.textAlign = 'center';
				drawText('Score: ' + handleLargeNumber(gui.score), Math.round(_x17 + _len3 / 2) + 0.5, Math.round(_y3 + _height2 - 4) + 0.5, _height2 - 2, color.guiwhite);
				ctx.lineWidth = 4;
				drawText(strDecodeUTF16(player.name), Math.round(_x17 + _len3 / 2) + 0.5, Math.round(_y3 - 10 - _vspacing2) + 0.5, 32, color.guiwhite);
			}

			{
				...
				var _height3 = _len4;
				var _x18 = global.screenWidth - _len4 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x18 + _i6 * _len4 / _W, _y4 + j++ * _height3 / _H, _len4 / _W, _height3 / _H);
					});
					_i6++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x18, _y4, _len4, _height3);
				ctx.globalAlpha = 0.8;
				minimap.forEach(function (o) {
					ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
					drawGuiRect(_x18 + o[0] / global.gameWidth * _len4, _y4 + o[1] / global.gameHeight * _height3, 1, 1);
				});
				ctx.globalAlpha = 1;
				ctx.lineWidth = 1;
				ctx.strokeStyle = color.black;
				drawGuiRect( // My position
				_x18 + player.x / global.gameWidth * _len4 - 1, _y4 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				drawGuiRect(_x18, _y4, _len4, _height3, true); // Border

				timingGraph(t, _x18, _y4 - 40, _len4, 30);

				ctx.textAlign = 'right';
				drawText('dddd: ' + metrics.rendergap + 'ms', _x18 + _len4, _y4 - 50 - 5 * 14, 10, color.guiwhite);
				drawText('Prediction: ' + t + 'ms', _x18 + _len4, _y4 - 50 - 4 * 14, 10, color.guiwhite);
				drawText('Update Rate: ' + metrics.updatetime + 'Hz', _x18 + _len4, _y4 - 50 - 3 * 14, 10, color.guiwhite);
				drawText('Latency: ' + metrics.latency + 'ms', _x18 + _len4, _y4 - 50 - 2 * 14, 10, color.guiwhite);
				drawText('Client FPS: ' + metrics.rendertime, _x18 + _len4, _y4 - 50 - 14, 10, color.guiwhite);
				drawText('Server Speed: ' + (100 * gui.fps).toFixed(2) + '%', _x18 + _len4, _y4 - 50, 10, color.guiwhite);
			}

			{
				// Draw leaderboard
				var _vspacing3 = 4;
				var _len5 = alcoveSize * global.screenWidth;
				var _height4 = 14;
				var _x19 = global.screenWidth - _len5 - spacing;
				var _y5 = spacing + _height4 + 7;
				ctx.lineWidth = 4;
				ctx.textAlign = 'center';
				drawText('Leaderboard:', Math.round(_x19 + _len5 / 2) + 0.5, Math.round(_y5 - 6) + 0.5, _height4 + 4, color.guiwhite);
				var _i7 = 0;
				leaderboard.forEach(function (entry) {
					leaderboardScore[_i7] += (entry.score - leaderboardScore[_i7]) / 20;
					drawBar(_x19, _x19 + _len5, _y5 + _height4 / 2, _height4, color.black);
					drawBar(_x19, _x19 + _len5, _y5 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x19, _x19 + _len5 * shift, _y5 + _height4 / 2, _height4 - 3.5, getColor(entry.bar));
					...
					drawText(txt + lab + ': ' + handleLargeNumber(Math.round(leaderboardScore[_i7++])), Math.round(_x19 + _len5 / 2) + 0.5, Math.round(_y5 + _height4 - 5) + 0.5, _height4 - 5, color.guiwhite);
					...
					var picture = getEntityImageFromMockup(entry.index, entry.color),
					    ...
					    xx = _x19 - 1.5 * _height4 - scale * position.middle.x * 0.707,
					    yy = _y5 + 0.5 * _height4 + scale * position.middle.x * 0.707;
					drawEntity(xx, yy, picture, 1 / scale, scale * scale / picture.realSize, -Math.PI / 4, true);
...

			{
				...
				if (gui.upgrades.length > 0) {
					...
					var _len6 = alcoveSize * global.screenWidth / 2 * 1;
					var _height5 = _len6;
					var _x20 = spacing;
					var _y6 = spacing;
					var _ticker = 0;
					...
					gui.upgrades.forEach(function (model) {
						...
						drawGuiRect(_x20, _y6, _len6, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x20, _y6, _len6, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x20, _y6 + _height5 * 0.6, _len6, _height5 * 0.4);
						ctx.globalAlpha = 1;

						// Find offset location with rotation
						var picture = getEntityImageFromMockup(model[0], model[1]),
						    position = mockups[model[0]].position,
						    scale = 0.6 * _len6 / position.axis,
						    xx = _x20 + 0.5 * _len6 - scale * position.middle.x * Math.cos(upgradeSpin),
						    yy = _y6 + 0.5 * _height5 - scale * position.middle.x * Math.sin(upgradeSpin);
						drawEntity(xx, yy, picture, 1, scale / picture.realSize, upgradeSpin, true);

						ctx.textAlign = 'center'; // Tank name
						ctx.lineWidth = 2;
						drawText(picture.name, Math.round(_x20 + 0.9 * _len6 / 2) + 0.5, Math.round(_y6 + _height5 - 6) + 0.5, _height5 / 8 - 3, color.guiwhite);
						ctx.textAlign = 'right'; // Upgrade key
						drawText('[' + getClassUpgradeKey(_ticker) + ']', Math.round(_x20 + _len6 - 4) + 0.5, Math.round(_y6 + _height5 - 6) + 0.5, _height5 / 8 - 3, color.guiwhite);

						ctx.strokeStyle = color.black; // Already incremented, see above
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x20, _y6, _len6, _height5, true); // Border

						if (_ticker++ % 2) {
							_y6 -= _height5 + internalSpacing;
							_x20 += _len6 + internalSpacing;
						} else {
							_y6 += _height5 + internalSpacing;
						}
					});
	...
	function gameDrawBeforeStart() {
		...
		ctx.textAlign = 'center';
		ctx.lineWidth = 4;
		drawText('Connecting...', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite);
		drawText(global.messages, global.screenWidth / 2, global.screenHeight / 2 + 30, 15, color.teal);
	}
